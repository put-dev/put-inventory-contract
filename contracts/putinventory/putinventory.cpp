...
ACTION putinventory::insertkey(const name& owner, const uint16_t& tag_id, const string& key, const string& value)
{
    ...
    const auto& keyval_itr = keyvals_index.find( putinventory::get_checksum256_key(to_string(tag_id) + "-" + key) );
...
ACTION putinventory::updatekey(const name& owner, const uint16_t& tag_id, const string& key, const string& value)
{
    require_auth( owner );
    keyvals keyvalstable(_self, owner.value);
    const auto& keyvals_index = keyvalstable.get_index<name("bycatkey")>();
    const auto& keyval = keyvals_index.get( putinventory::get_checksum256_key(to_string(tag_id) + "-" + key), "key not found" );
    keyvalstable.modify(keyval, owner, [&]( auto& d ) {
        d.value = value;
    });
}

ACTION putinventory::rekey(const name& owner, const uint16_t& tag_id, const string& key, const string& new_key)
{
    require_auth( owner );
    check(key != new_key, "cannot rekey to same key");

    const auto& cat_str = to_string(tag_id);
    keyvals keyvalstable(_self, owner.value);
    const auto& keyvals_index = keyvalstable.get_index<name("bycatkey")>();
...
ACTION putinventory::deletekey(const name& owner, const uint16_t& tag_id, const string& key)
{
    require_auth( owner );
    keyvals keyvalstable(_self, owner.value);
    const auto& keyvals_index = keyvalstable.get_index<name("bycatkey")>();
